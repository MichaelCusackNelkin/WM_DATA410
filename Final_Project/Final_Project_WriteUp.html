...
<p>I found out from a NYTimes article that New york City recently passed a law allowing citizens to submit a three minute video of a commercial vehicle idling in the street. Since the city fines vehicles for idling, they offer 50% of the fines to the citzen as incentive. The company’s label must be easily visible and the vehicle must be "noticeably" idling in order to get any penalties. A regualr person could expect around $80 per video, and NYC has thousands of idling trucks all over the city at any given time. Then the story continued, saying that the city was leaving millions of dollars of fines on the table because they didn’t have the work force to process all the fines. When I read that, my first thought was that this is a reasonably solvable video-classification problem. It should be possible to verify that a video is of an idling truck and then pick out the company label with a text processor. So, I am interested in developing a video classifcation algorithm to hunt idling commercial trucks in NYC. The final goal of this research is to develope an easy-to-use app so that users can take a video adn automatically send a fine to the city government, and get paid as fast as possible. To start, I am building a video classifcation algorithm with the Highway Traffic Videos Dataset at <a href="https://www.kaggle.com/datasets/aryashah2k/highway-traffic-videos-dataset">https://www.kaggle.com/datasets/aryashah2k/highway-traffic-videos-dataset</a>. This will begin the first phase of the project, learning how to classify videos.</p>
...
<p>As this dataset is 63MB of video data (254 videos), I used Google Colab Pro GPUs to accelerate the training stage. Without this, none of the below methods would’ve run in any useful amount of time.</p>
...
<p>I ended up making six hypertuned models, and found them to be somewhat inconsistent. In the end, the best hypertuned model had this configuration:</p>
